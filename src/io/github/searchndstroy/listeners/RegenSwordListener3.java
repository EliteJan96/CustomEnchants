...
public class RegenSwordListener3 implements Listener {
	
	   Main plugin;
	    public RegenSwordListener3 (Main instance) {
	...
	public void onEntityDamageByEntityEvent3(EntityDamageByEntityEvent event) {
		
		Entity attacker = (Entity) event.getDamager();
		Entity defender = (Entity) event.getEntity();
		int seconds0 = plugin.getCustomConfig().getInt("Regen.IV.seconds");
		int seconds = seconds0*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Regen.IV.amplifier");
        ...
        if(attacker instanceof Player) {
        	...
        	if(defender instanceof Player) {
	    		...
	    		if(((HumanEntity) attacker).getItemInHand().getItemMeta().hasLore()) {
	    			...
	    			if(((HumanEntity) attacker).getItemInHand().getItemMeta().getLore().get(0).equals("§r§7Regen IV")) {
