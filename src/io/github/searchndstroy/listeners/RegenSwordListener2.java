...
public class RegenSwordListener2 implements Listener {
	
	   Main plugin;
	    public RegenSwordListener2 (Main instance) {
	...
	public void onEntityDamageByEntityEvent2(EntityDamageByEntityEvent event) {
		...
		int seconds0 = plugin.getCustomConfig().getInt("Regen.III.seconds");
		int seconds = seconds0*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Regen.III.amplifier");
        ...
        if(attacker instanceof Player) {
        	 ...
        	 if(defender instanceof Player) {
	    		...
	    		if(((HumanEntity) attacker).getItemInHand().getItemMeta().hasLore()) {
	    			...
	    			if(((HumanEntity) attacker).getItemInHand().getItemMeta().getLore().get(0).equals("§r§7Regen III")) {
