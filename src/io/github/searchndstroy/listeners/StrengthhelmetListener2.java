...
public class StrengthhelmetListener2 implements Listener {
	
	   Main plugin;
	    public StrengthhelmetListener2 (Main instance) {
	...
	public void onEntityDamageByEntityEvent(EntityDamageByEntityEvent event) {
		...
		int seconds0 = plugin.getCustomConfig().getInt("Strength.III.seconds");
		int seconds = seconds0*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Strength.III.amplifier");
			...
			if(defender instanceof Player) {
				...
				if (!(((HumanEntity) attacker).getInventory().getHelmet() != null)) {
					...
					if(((HumanEntity) attacker).getInventory().getHelmet().getItemMeta().hasLore()) {
						...
						if (((HumanEntity) attacker).getInventory().getHelmet().getItemMeta().getLore().get(0).equals("§r§7Strength III")) {
