...
public class SpeedbootsListener2 implements Listener {
	
	   Main plugin;
	    public SpeedbootsListener2 (Main instance) {
	...
	public void  onPlayerMoveEvent(PlayerMoveEvent event) {
		...
		int seconds0 = plugin.getCustomConfig().getInt("Speed.III.seconds");
		int seconds = seconds0*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Speed.III.amplifier");
		...
		if (player instanceof Player) {
			...
			if(!(((HumanEntity) player).getInventory().getBoots() != null)) {
				...
				if(((HumanEntity) player).getInventory().getBoots().getItemMeta().hasLore()) {
					...
					if(((HumanEntity) player).getInventory().getBoots().getItemMeta().getLore().get(0).equals("§r§7Speed III")) {
