...
public class InvisibilityswordListener1 implements Listener {
	
    Main plugin;
    public InvisibilityswordListener1 (Main instance) {
	...
	public void onEntityDamageByEntityEvent(EntityDamageByEntityEvent event) {
		...
		int seconds0 = plugin.getCustomConfig().getInt("Invisibility.II.seconds");
		int seconds = seconds0*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Invisibility.II.amplifier");
			...
			if(defender instanceof Player) {
				...
				if (!((HumanEntity) attacker).getInventory().getItemInHand().equals(null) || ((HumanEntity) attacker).getInventory().getItemInHand().equals(Material.AIR)) {
					...
					if(((HumanEntity) attacker).getItemInHand().getItemMeta().hasLore()) {
						...
						if(((HumanEntity) attacker ).getItemInHand().getItemMeta().getLore().get(0).equals("§r§7Invisibility II")) {
							
							((LivingEntity) attacker).addPotionEffect(new PotionEffect (PotionEffectType.INVISIBILITY, seconds, amplifier));
						}
