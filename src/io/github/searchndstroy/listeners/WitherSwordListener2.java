...
public class WitherSwordListener2 implements Listener {
	
	   Main plugin;
	    public WitherSwordListener2 (Main instance) {
	...
	public void onEntityDamageByEntityEvent12(EntityDamageByEntityEvent event) {
		
		Entity attacker = (Entity) event.getDamager();
		Entity defender = (Entity) event.getEntity();
		int seconds0 = plugin.getCustomConfig().getInt("Wither.III.seconds");
		int seconds = seconds0*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Wither.III.amplifier");
		...
		if(attacker instanceof Player) {
			...
			if(defender instanceof Player) {
				...
				if(((HumanEntity) attacker).getItemInHand().getItemMeta().hasLore()) {
					...
					if(((HumanEntity) attacker).getItemInHand().getItemMeta().getLore().get(0).equals("§r§7Wither III")) {
						
						((LivingEntity) defender).addPotionEffect(new PotionEffect (PotionEffectType.WITHER, seconds, amplifier)); {
