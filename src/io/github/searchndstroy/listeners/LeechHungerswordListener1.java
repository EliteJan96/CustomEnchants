...
public class LeechHungerswordListener1 implements Listener {
	...
	public void onEntityDamageByEntityEvent30(EntityDamageByEntityEvent event) {
		...
		int seconds0 = plugin.getCustomConfig().getInt("Leechhunger.II.seconds.saturation");
		int seconds1 = plugin.getCustomConfig().getInt("Leechhunger.II.seconds.hunger");
		int seconds = seconds0*20;
		int seconds01 = seconds1*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Leechhunger.II.amplifier.saturation");
		int amplifier1 = plugin.getCustomConfig().getInt("Leechhunger.II.amplifier.hunger");
			...
			if(defender instanceof Player) {
				...
				if (!((HumanEntity) attacker).getInventory().getItemInHand().equals(null) || ((HumanEntity) attacker).getInventory().getItemInHand().equals(Material.AIR)) {
					...
					if(((HumanEntity) attacker).getItemInHand().getItemMeta().hasLore()) {
						...
						if(((HumanEntity) attacker ).getItemInHand().getItemMeta().getLore().get(0).equals("§r§7LeechHunger II")) {
