...
public class LeechHungerswordListener2 implements Listener {
	
    Main plugin;
    public LeechHungerswordListener2 (Main instance) {
	...
	public void onEntityDamageByEntityEvent31(EntityDamageByEntityEvent event) {
		...
		int seconds0 = plugin.getCustomConfig().getInt("Leechhunger.III.seconds.saturation");
		int seconds1 = plugin.getCustomConfig().getInt("Leechhunger.III.seconds.hunger");
		int seconds = seconds0*20;
		int seconds01 = seconds1*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Leechhunger.III.amplifier.saturation");
		int amplifier1 = plugin.getCustomConfig().getInt("Leechhunger.III.amplifier.hunger");
			...
			if(defender instanceof Player) {
				...
				if (!((HumanEntity) attacker).getInventory().getItemInHand().equals(null) || ((HumanEntity) attacker).getInventory().getItemInHand().equals(Material.AIR)) {
					...
					if(((HumanEntity) attacker).getItemInHand().getItemMeta().hasLore()) {
						...
						if(((HumanEntity) attacker ).getItemInHand().getItemMeta().getLore().get(0).equals("§r§7LeechHunger III")) {
