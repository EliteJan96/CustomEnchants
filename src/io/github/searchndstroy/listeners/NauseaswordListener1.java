...
public class NauseaswordListener1 implements Listener {
	
	   Main plugin;
	    public NauseaswordListener1 (Main instance) {
	...
	public void onEntityDamageByEntityEvent24(EntityDamageByEntityEvent event) {
		
		Entity attacker = (Entity) event.getDamager();
		Entity defender = (Entity) event.getEntity();
		int seconds0 = plugin.getCustomConfig().getInt("Nausea.II.seconds");
		int seconds = seconds0*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Nausea.II.amplifier");
			...
			if(defender instanceof Player) {
				...
				if (!((HumanEntity) attacker).getInventory().getItemInHand().equals(null) || ((HumanEntity) attacker).getInventory().getItemInHand().equals(Material.AIR)) {
					...
					if(((HumanEntity) attacker).getItemInHand().getItemMeta().hasLore()) {
						...
						if(((HumanEntity) attacker ).getItemInHand().getItemMeta().getLore().get(0).equals("§r§7Nausea II")) {
							
							((LivingEntity) defender).addPotionEffect(new PotionEffect (PotionEffectType.CONFUSION, seconds, amplifier));
						}
