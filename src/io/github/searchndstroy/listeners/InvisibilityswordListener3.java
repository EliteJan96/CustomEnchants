...
public class InvisibilityswordListener3 implements Listener {
	
    Main plugin;
    public InvisibilityswordListener3 (Main instance) {
	...
	public void onEntityDamageByEntityEvent(EntityDamageByEntityEvent event) {
		...
		int seconds0 = plugin.getCustomConfig().getInt("Invisibility.IV.seconds");
		int seconds = seconds0*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Invisibility.IV.amplifier");
			...
			if(defender instanceof Player) {
				...
				if (!((HumanEntity) attacker).getInventory().getItemInHand().equals(null) || ((HumanEntity) attacker).getInventory().getItemInHand().equals(Material.AIR)) {
					...
					if(((HumanEntity) attacker).getItemInHand().getItemMeta().hasLore()) {
						...
						if(((HumanEntity) attacker ).getItemInHand().getItemMeta().getLore().get(0).equals("§r§7Invisibility IV")) {
