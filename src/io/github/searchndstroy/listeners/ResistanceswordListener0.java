...
public class ResistanceswordListener0 implements Listener {
	
	   Main plugin;
	    public ResistanceswordListener0 (Main instance) {
	...
	public void onEntityDamageByEntityEvent28(EntityDamageByEntityEvent event) {
		
		Entity attacker = (Entity) event.getDamager();
		Entity defender = (Entity) event.getEntity();
		int seconds0 = plugin.getCustomConfig().getInt("Resistance.I.seconds");
		int seconds = seconds0*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Resistance.I.amplifier");
			...
			if(defender instanceof Player) {
				...
				if (!((HumanEntity) attacker).getInventory().getItemInHand().equals(null) || ((HumanEntity) attacker).getInventory().getItemInHand().equals(Material.AIR)) {
					...
					if(((HumanEntity) attacker).getItemInHand().getItemMeta().hasLore()) {
						...
						if(((HumanEntity) attacker ).getItemInHand().getItemMeta().getLore().get(0).equals("§r§7Resistance I")) {
							
							((LivingEntity) attacker).addPotionEffect(new PotionEffect (PotionEffectType.DAMAGE_RESISTANCE, seconds, amplifier));
						}
