...
public class WitherSwordListener0 implements Listener {
	...
	public void onEntityDamageByEntityEvent10(EntityDamageByEntityEvent event) {
		...
		int seconds0 = plugin.getCustomConfig().getInt("Wither.I.seconds");
		int seconds = seconds0*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Wither.I.amplifier");
		...
		if(attacker instanceof Player) {
			...
			if(defender instanceof Player) {
				...
				if(((HumanEntity) attacker).getItemInHand().getItemMeta().hasLore()) {
					 ...
					 if(((HumanEntity) attacker).getItemInHand().getItemMeta().getLore().get(0).equals("§r§7Wither I")) {
