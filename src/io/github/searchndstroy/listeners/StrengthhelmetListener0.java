...
public class StrengthhelmetListener0 implements Listener {
	
	   Main plugin;
	    public StrengthhelmetListener0 (Main instance) {
			...
			if(defender instanceof Player) {
				...
				if (!(((HumanEntity) attacker).getInventory().getHelmet() != null)) {
					...
					if(((HumanEntity) attacker).getInventory().getHelmet().getItemMeta().hasLore()) {
						...
						if (((HumanEntity) attacker).getInventory().getHelmet().getItemMeta().getLore().get(0).equals("§r§7Strength I")) {
