...
public class PoisonSwordListener3 implements Listener {
	...
	public void onEntityDamageByEntityEvent8(EntityDamageByEntityEvent event) {
		...
		int seconds0 = plugin.getCustomConfig().getInt("Poison.IV.seconds");
		int seconds = seconds0*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Poison.IV.amplifier");
		...
		if(attacker instanceof Player) {
			...
			if(defender instanceof Player) {
				...
				if(((HumanEntity) attacker).getItemInHand().getItemMeta().hasLore()) {
					...
					if(((HumanEntity) attacker).getItemInHand().getItemMeta().getLore().get(0).equals("§r§7Poison IV")) {
