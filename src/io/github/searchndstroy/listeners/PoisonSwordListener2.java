...
public class PoisonSwordListener2 implements Listener {
	
	   Main plugin;
	    public PoisonSwordListener2 (Main instance) {
	...
	public void onEntityDamageByEntityEvent7(EntityDamageByEntityEvent event) {
		
		Entity attacker = (Entity) event.getDamager();
		Entity defender = (Entity) event.getEntity();
		int seconds0 = plugin.getCustomConfig().getInt("Poison.III.seconds");
		int seconds = seconds0*20;
		int amplifier0 = plugin.getCustomConfig().getInt("Poison.III.amplifier");
		...
		if(attacker instanceof Player) {
			...
			if(defender instanceof Player) {
				...
				if(((HumanEntity) attacker).getItemInHand().getItemMeta().hasLore())  {
					...
					if(((HumanEntity) attacker).getItemInHand().getItemMeta().getLore().get(0).equals("§r§7Poison III")) {
